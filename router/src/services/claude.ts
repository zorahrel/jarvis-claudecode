 ...
 * Escape hatch for catastrophic cases — for routine post-auth / post-
 * logout / post-restart updates use `reconnectMcpInLiveSessions()`
 * instead, which keeps the session alive and only refreshes the MCP
 * transport in-place.
 *
 * "Could use" means: agent has `fullAccess: true` OR agent's `tools`
 * array contains `mcp:<mcpName>`. Returns count of sessions killed.
 */
...
/**
 * In-place reconnect of a single MCP server across every live SDK session
 * that may use it. Replaces the brutal kill+respawn after dashboard MCP
 * mutations (authenticate / disconnect / restart): the live session stays
 * alive (warm prompt cache, in-flight tool calls, subagents, conversation
 * state — all preserved), only the MCP transport is rebuilt.
 *
 * Uses the SDK's native `Query.reconnectMcpServer(name)` (the same
 * mechanism the fcache watcher above uses for token-rotation events).
 *
 * Returns:
 *   { reconnected, failed, skipped } — for telemetry / dashboard toasts.
 *   `skipped` is sessions whose agent doesn't use this MCP at all.
 */
export async function reconnectMcpInLiveSessions(mcpName: string): Promise<{ reconnected: number; failed: number; skipped: number }> {
  const wanted = `mcp:${mcpName}`;
  let reconnected = 0;
  let failed = 0;
  let skipped = 0;
  for (const s of Array.from(sessions.values())) {
    ...
    try {
      await s.q.reconnectMcpServer(mcpName);
      reconnected++;
      log.info({ sessionKey: s.sessionKey, mcp: mcpName }, "MCP reconnected in-place after dashboard action");
    } catch (err) {
      failed++;
      log.warn({ sessionKey: s.sessionKey, mcp: mcpName, err: String(err) }, "MCP in-place reconnect failed; session keeps stale transport");
    }
  }
  if (reconnected + failed > 0) log.info({ mcpName, reconnected, failed, skipped }, "MCP in-place reconnect summary");
