...
async function askClaudeInternal(
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      ...
      let pp = getOrCreateProcess(key, model, agent.workspace, tools, agent.effort, agent.fullAccess, agent.env, agent.inheritUserScope);

      // If model changed (fallback), need new process
      if (pp.model !== model) {
        killProcess(pp);
        processes.delete(key);
        pp = getOrCreateProcess(key, model, agent.workspace, tools, agent.effort, agent.fullAccess, agent.env, agent.inheritUserScope);
      }
