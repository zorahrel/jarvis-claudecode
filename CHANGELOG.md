...
- **Keep-warm auto-respawn.** `keepWarm: true` in `agent.yaml` makes the router
  immediately respawn a fresh idle session when the inactivity timer fires, so
  all messages after the first have zero cold-start latency.
- **ps cache for sidecar discovery.** `findChildClaudePids()` caches the `ps`
  output for 5s, avoiding redundant invocations on rapid successive spawns.
...
- **WhatsApp attachment self-loop.** Sent attachment message IDs are now tracked
  in `sentMsgIds` so the inbound echo is recognized as bot-sent and skipped.
- **Dashboard `inactivityExpiresAt`** now reflects the per-agent timeout instead
